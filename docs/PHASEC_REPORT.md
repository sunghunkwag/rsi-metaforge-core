# Phase C Report — Speculative Ledger Extension (Rule 3 for the Top Core)
...
## C1 — Single permanent-installation call site

`_permanent_install(rs, searcher, table)` is now the only place a live
run's `rs.searcher` is assigned. `_install` (screen path) and the
meta-gate bundle path both funnel through it, each reachable only after
sealed holdout + counterfactual gate passage; `restore_runstate`'s
assignment is deserialization of already-certified state. The refactor is
behavior-preserving (verified: identical adoption digest, adopted sets,
and event streams vs Phase B). Audited by
`test_permanent_install_single_call_site` (AST scan: `.searcher`
assignment sites must be exactly `{_permanent_install, restore_runstate}`).

## C2 — Byte-identical rollback

`_searcher_state_hash` serializes the full searcher (macros, weights,
version, extended ops, capacity) and hashes it. `speculative_meta_gate`
records the hash before speculation; on gate rejection it asserts the
post-state hash equals the pre-state hash (a violation raises — a failed
rollback is a hard error, not a log line) and stores both hashes in the
ledger entry. Verified by `test_speculative_rollback_restores_certified_state`
on both a rejected rung (hash-identical restore, empty scratch) and an
adopted rung (new certified state).

## C3 — Scopes, ledger, budget

- Scope (a): `attempt_capacity_growth` routes every capacity rung through
  `speculative_meta_gate(..., "capacity_rung")`.
- Scope (b): `run_system` classifies each ordinary proposal with
  `_proposal_is_generated_derived` (any new macro carrying a generated
  parent tid); such batches route through
  `speculative_meta_gate(..., "generated_macro_batch")`. Designer-only
  batches keep the pre-existing direct gate path (scope discipline).
- The ledger is the pre-existing SECTION 24 `SpeculativeLedger`
  (certified/scratch/history), used literally: speculate into scratch,
  gate decides, finalize moves-or-pops, `rollback_all_scratch` cascades
  the remainder; residue count recorded (always 0 — asserted by test).
- Budget: `SPECULATION_BUDGET_PER_WAVE = 2` (one growth rung + one
  generated batch is the natural per-wave maximum). Over-budget attempts
  return "not measured" (`None`) — they are logged
  (`SPECULATION_BUDGET_EXHAUSTED`), produce no gate record and no ledger
  entry, and do **not** enter `rejected_digests` (a deferred candidate is
  not a rejected one).
- The meta_gate itself is untouched (GR2).

## Measured behavior on the default run

Adaptive arm at identical seeds/budgets: adoption digest, adopted sets,
and events **identical to Phase B** (`42576f55a5cec8b6`; designer 22 /
generated 4). The ledger records real speculative traffic: the wave-1
generated-derived batch was adopted (certified hash advanced), and later
generated-derived batches were rejected with hash-verified clean
rollbacks (`pre_hash == post_hash`, scratch residue 0). Frozen arm:
bit-identical to Phase 0 (digest `e5dc308324bb10f6`), empty speculation
ledger. Two consecutive runs of each arm byte-identical (GR3).

## C4 — New tests (5 added; none modified)

1. `test_permanent_install_single_call_site` — AST audit of C1.
2. `test_generated_derived_classification` — scope (b) predicate.
3. `test_speculative_rollback_restores_certified_state` — C2 on both a
   rejected and an adopted rung, with ledger history and residue checks.
4. `test_speculation_budget_bounded_per_wave` — two measured attempts per
   wave, third refused without measurement, budget resets next wave.
5. `test_speculation_ledger_deterministic` — two-run byte-identical
