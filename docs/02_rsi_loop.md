...
The main RSI-style loop is implemented by [`run_system`](../rsi_levels_metaforge_unified.py#L1921). Each wave runs task search through [`run_wave`](../rsi_levels_metaforge_unified.py#L1743), updates drift and negative-gram bookkeeping, optionally mints frontier tasks, proposes searcher changes through [`propose_improvement`](../rsi_levels_metaforge_unified.py#L1377), and sends those changes through [`meta_gate`](../rsi_levels_metaforge_unified.py#L1522).
...
- A synthesized token program for a sealed task, generated by [`synthesize`](../rsi_levels_metaforge_unified.py#L1104).
- A searcher-level macro or weight update represented by [`ImprovementProposal`](../rsi_levels_metaforge_unified.py#L1308).
- A forged primitive proposed by the self-forge battery through [`forge_synthesize`](../rsi_levels_metaforge_unified.py#L7541).
- A file-world skill or repair candidate generated from residues through [`fw_propose_from_residues`](../rsi_levels_metaforge_unified.py#L8927).
- A general-domain macro mined from solved expressions through [`gd_mine_macros`](../rsi_levels_metaforge_unified.py#L35355).
...
For task programs, acceptance means a candidate passes train re-verification plus both sealed gates in [`run_wave`](../rsi_levels_metaforge_unified.py#L1797) and is stored in `RunState.adopted_tokens`.
...
- `GATE_FAIL` and `CF_GATE_CATCH` events from [`run_wave`](../rsi_levels_metaforge_unified.py#L1801).
- `META_REJECT` records from [`meta_gate`](../rsi_levels_metaforge_unified.py#L1547).
- Duplicate proposal suppression through `RunState.rejected_digests` in [`run_system`](../rsi_levels_metaforge_unified.py#L1953).
...
- [`lineage_report`](../rsi_levels_metaforge_unified.py#L1966), which summarizes macro lineage and deeper reuse.
- [`runstate_summary`](../rsi_levels_metaforge_unified.py#L2033), which serializes accepted programs, gate records, residues, rejected digests, TCCI reports, and adoption digest.
