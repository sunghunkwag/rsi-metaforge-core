...
The main RSI-style loop is implemented by [`run_system`](../rsi_levels_metaforge_unified.py#L2235). Each wave runs task search through [`run_wave`](../rsi_levels_metaforge_unified.py#L2055), updates drift and negative-gram bookkeeping, optionally mints frontier tasks, proposes searcher changes through [`propose_improvement`](../rsi_levels_metaforge_unified.py#L1505), and sends those changes through [`meta_gate`](../rsi_levels_metaforge_unified.py#L1665).
...
- A synthesized token program for a sealed task, generated by [`synthesize`](../rsi_levels_metaforge_unified.py#L1227).
- A searcher-level macro or weight update represented by [`ImprovementProposal`](../rsi_levels_metaforge_unified.py#L1434).
- A forged primitive proposed by the self-forge battery through [`forge_synthesize`](../rsi_levels_metaforge_unified.py#L7938).
- A file-world skill or repair candidate generated from residues through [`fw_propose_from_residues`](../rsi_levels_metaforge_unified.py#L9324).
- A general-domain macro mined from solved expressions through [`gd_mine_macros`](../rsi_levels_metaforge_unified.py#L37605).
...
For task programs, acceptance means a candidate passes train re-verification plus both sealed gates in [`run_wave`](../rsi_levels_metaforge_unified.py#L2114) and is stored in `RunState.adopted_tokens`.
...
- `GATE_FAIL` and `CF_GATE_CATCH` events from [`run_wave`](../rsi_levels_metaforge_unified.py#L2116).
- `META_REJECT` records from [`meta_gate`](../rsi_levels_metaforge_unified.py#L1696).
- Duplicate proposal suppression through `RunState.rejected_digests` in [`run_system`](../rsi_levels_metaforge_unified.py#L2280).
...
- [`lineage_report`](../rsi_levels_metaforge_unified.py#L2301), which summarizes macro lineage and deeper reuse.
- [`runstate_summary`](../rsi_levels_metaforge_unified.py#L2397), which serializes accepted programs, gate records, residues, rejected digests, TCCI reports, and adoption digest.
