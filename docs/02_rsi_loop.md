...
The main RSI-style loop is implemented by [`run_system`](../rsi_metaforge_core.py). Each wave runs task search through [`run_wave`](../rsi_metaforge_core.py), updates drift and negative-gram bookkeeping, optionally mints frontier tasks, proposes searcher changes through [`propose_improvement`](../rsi_metaforge_core.py), and sends those changes through [`meta_gate`](../rsi_metaforge_core.py).
...
- A synthesized token program for a sealed task, generated by [`synthesize`](../rsi_metaforge_core.py).
- A searcher-level macro or weight update represented by [`ImprovementProposal`](../rsi_metaforge_core.py).
- A forged primitive proposed by the self-forge battery through [`forge_synthesize`](../rsi_metaforge_core.py).
- A file-world skill or repair candidate generated from residues through [`fw_propose_from_residues`](../rsi_metaforge_core.py).
- A general-domain macro mined from solved expressions through [`gd_mine_macros`](../rsi_metaforge_core.py).
...
For task programs, acceptance means a candidate passes train re-verification plus both sealed gates in [`run_wave`](../rsi_metaforge_core.py) and is stored in `RunState.adopted_tokens`.

For searcher modifications, acceptance means [`meta_gate`](../rsi_metaforge_core.py) finds at least one newly reachable previously unsolved task whose program passes the external holdout and counterfactual gates. Accepted macros are installed with usage-based credit; unused riders can be dropped before integration.
...
- `GATE_FAIL` and `CF_GATE_CATCH` events from [`run_wave`](../rsi_metaforge_core.py).
- `META_REJECT` records from [`meta_gate`](../rsi_metaforge_core.py).
- Duplicate proposal suppression through `RunState.rejected_digests` in [`run_system`](../rsi_metaforge_core.py).
...
- [`lineage_report`](../rsi_metaforge_core.py), which summarizes macro lineage and deeper reuse.
- [`runstate_summary`](../rsi_metaforge_core.py), which serializes accepted programs, gate records, residues, rejected digests, TCCI reports, and adoption digest.
