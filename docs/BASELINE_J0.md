# Phase J0 — Cold-Start Baseline

Recorded per DIRECTIVE v5 §2. All numbers below were produced from a clean
git worktree pinned at the `main` tip this session started from; nothing in
this document depends on any Phase J change.

## 1. Provenance

- **Baseline `main` tip:** `3a4aac3e99dd716a2042c773fa05d1caec42fa18`
  ("Merge pull request #13"). During the session `main` advanced by
  docs-only merges; `git diff 3a4aac3 da100a7 -- rsi_levels_metaforge_unified.py`
  is empty, so the runtime under test is byte-identical to the pre-Phase-J
  `main` tip.
- **Environment:** Linux 6.18.5, CPython 3.11.15, pytest 9.1.1, 4 CPU cores,
  15 GB RAM (remote execution container).
- **Working directory:** a dedicated clean worktree (`git worktree add
  <dir> 3a4aac3`), so baseline runs never observe Phase J working-tree
  edits.

## 2. Run-order finding (recorded as measured)

A bare-clone invocation of both harnesses — before generating any evidence
artifacts — yields **137 passed, 10 failed** in both the native registry and
pytest, and the two harnesses agree exactly on the failing set. All ten
failures are artifact-dependency tests that say so explicitly
("artifact missing; run --mode <battery> first"):

```
FAIL test_deep_scan_artifact_certifies_walls: deep scan artifact missing; run --mode horizon-scan first
FAIL test_forge_results_walls_fall_through_sealed_gates: forge artifact missing; run --mode forge-battery first
FAIL test_forge_downstream_mint_solvable_only_with_forged_ops: forge artifact missing
FAIL test_fw_results_adaptive_beats_frozen_on_hidden: artifact missing; run --mode file-battery first
FAIL test_fw_skills_do_not_encode_eval_ids: artifact missing
FAIL test_fw_curriculum_round_two_improves: artifact missing
FAIL test_fw_repair_single_line_patch_passes_hidden: artifact missing
FAIL test_fw_traces_deterministic_and_archive_roundtrip: archive missing; run file-battery
FAIL test_cfs_results_walls_and_propagation: cfs artifact missing; run --mode cfs-battery first
FAIL test_expansion_results_consistent: expansion artifact missing; run --mode expansion-battery
```

This matches the Full Evidence CI workflow, which generates every battery
artifact before running `--mode test`. The canonical baseline below
replicates that order and is fully green. Not a pre-existing red.

## 3. Exact commands (canonical order, as executed)

```
python3 rsi_levels_metaforge_unified.py --mode file-battery
python3 rsi_levels_metaforge_unified.py --mode forge-battery
python3 rsi_levels_metaforge_unified.py --mode horizon-scan
python3 rsi_levels_metaforge_unified.py --mode cfs-battery
python3 rsi_levels_metaforge_unified.py --mode expansion-battery
python3 rsi_levels_metaforge_unified.py --mode grammar-battery
python3 rsi_levels_metaforge_unified.py --mode grammar2-battery
python3 rsi_levels_metaforge_unified.py --mode test
python3 -m pytest rsi_levels_metaforge_unified.py -q
```

Operational note, recorded for reproducibility: the execution container was
reclaimed twice mid-chain during this session; the chain was made resumable
(completed batteries detected by their artifacts and skipped) and re-run to
completion from the clean worktree. Battery outputs are deterministic, and
the file-battery/forge-battery artifacts consumed by the final harness runs
were produced by unmodified baseline code.

## 4. Designer ledger at baseline (verified from the committed artifact)

From `docs/final_live_phaseI.json` (SHA-256
`1cffe630aa1d1e8aac27d132f6c9a7f63c870919162ead1ae025f8bb8463b95a`):

- **26 / 33 designer tasks solved** (adoption digest `e2c2893448da4cdf`):
  T00–T17, T19, T20, T23–T28.
- **OPEN (SEARCH/VOCAB attributed):** T18 `length_conditioned_sum`,
  T21 `last_quartile`, T22 `sum*second_largest`.
- **Certified infeasible in the base ISA:** T29 `count_above_threshold`,
  T30 `argmax_index`, T31 `first_greater_than_previous`,
  T32 `longest_increasing_run` — exact observational-equivalence
  certificates to expanded length 7 (length 8 swept complete), pinned by
  `test_closure_certificates_walls_blocked_le6`,
  `test_deep_scan_artifact_certifies_walls`, and the executed
  constructor lemma test.
- Frozen instruments verified by hash-pin tests in the green run below:
  `docs/frozen_holdout_phase0.json` (`527bb04d…da24`),
  `docs/ORDERING_SPEC.md` (`4b42e499…0dce`), the Phase D/G archives and
  anchor reports, and the anchor implementation source pin.

## 5. Native harness — full untruncated output
...
## 6. pytest harness — full untruncated output

```
........................................................................ [ 48%]
........................................................................ [ 97%]
...                                                                      [100%]
147 passed in 2066.19s (0:34:26)
```
