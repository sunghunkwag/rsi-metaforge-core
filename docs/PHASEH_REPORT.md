# Phase H Report — Acceptance Outputs

Phase H substance: `docs/UNION_H.md` (the 24-task union, citable, with
the G3-only artifact committed after two byte-identical reproductions
matching the cited digest), `docs/DISPLACEMENT_TRACE.md` (the collapse
as three verified mechanisms, including the digest-exact re-derivation
of the run's own skip digest `751d1479e8586590`), and
`docs/ORDERING_SPEC.md` (frozen, SHA-256
`4b42e49951e72a398191cbedb1d6f8e0de7240be7267772744358e69fced0dce`,
hash-pinned by test, with the worked dry-run). Zero mechanism changes;
one analysis helper and one test added.

Reconstruction determinism evidence:
- G3-only reproduction: two consecutive runs byte-identical; artifact
  equals the pre-existing development run byte-for-byte and reproduces
  digest `93338b61e81fe6e1`.
- Fragment census: two computations identical (also asserted by the new
  test, alongside the spec hash pin, artifact digest, and union
  consistency checks).
