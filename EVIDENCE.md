...
- an MDL master gate (one frozen coding formula judging all vocabulary growth, holdout-validated) through `ascent-l-battery`; a ledger-trained amortizer behind a strict passes-per-budget gate through `ascent-m-battery`; a frozen propositional proof kernel with a conjecture setter, countermodel refutations, and zero axiom adoptions through `ascent-n-battery`; the two-channel meta-improvement protocol (kernel-certified schemas + counterfactual A/B with an end-to-end extender record) through `ascent-o-battery`; and the ISA-extension coupling loop (a K8-gated extender feeding the task factory, closure trajectory read off the K10 ledger, with a designer-stocked crossing control) through `ascent-p-battery` — each run twice with digest and artifact equality asserted in CI
- a full built-in regression suite expected to report `286 passed, 0 failed`
...
- **P (ISA-extension coupling loop, digest `506107aacab14861`)**: the coupling loop feeds K8-gated ISA extension back into K's task factory so the closure itself can grow. Pre-registered question — revive or re-saturate — answered by measurement: from a base start under the frozen two-op catalog the trajectory RE-SATURATES at the frozen-K ceiling (coupling cum-crossed `[0,3,3,3,3,3,3]` = frozen 3, ISA never grows, at every mining-budget rung), while the extender still fires one mechanical `BCAST` request and K8 records an honest rejection with the ISA restored byte-exactly. A designer-stocked positive control certifies the mechanism (base passes 0 → both catalog ops admitted, 3 and 2 extended crossings). The frozen instrument budget (4000) sits above the extension-crossing search reach (~3000, verified by the control), so the null is a closure-shape result, not a budget artifact. Both outcomes were pre-declared wins. `docs/PREDICTIONS_P.md` → `docs/P_RESULT.md`.
